...
const DTLInfoDelay2: React.FC = () => {
  ...
  const renderLeftATTLine = () => {
    const circleSize = 10;
    const totalSpacing = circleSize + stationGap;
    const midMarginRight = 70.5;

    const DOT_R = 5;                 // <-- make it as big as you want
    const LINE_X = DOT_R + 1;        // keep line away from left edge
    const SVG_W  = LINE_X + DOT_R;   // width so dot isn't clipped horizontally
    const PAD_Y  = DOT_R;            // vertical padding so top/btm dots fit
    ...
    return stationsLeft.slice(0, -1).map((station, idx) => {
      ...
      if (!bothATT) return null;

      const startY = idx * totalSpacing + circleSize / 2;
      const endY   = (idx + 1) * totalSpacing + circleSize / 2;
      const segH   = endY - startY;

      const svgTop     = startY - PAD_Y;         // shift up by padding
      const svgHeight  = segH + PAD_Y * 2;       // add padding top+bottom
      ...
      return (
        <svg
          key={`att-left-${station.code}-${next.code}`}
          style={{
            position: 'absolute',
            right: midMarginRight,
            top: svgTop,
            pointerEvents: 'none',
            overflow: 'visible',
            zIndex: 2,
        ...
        >
          {/* dashed segment (inside padded area) */}
          <line
            x1={LINE_X}
            y1={PAD_Y}
            x2={LINE_X}
          ...
          />
          {showStartDot && <circle cx={LINE_X} cy={PAD_Y + 1} r={DOT_R} fill="black" />}
          {showEndDot   && <circle cx={LINE_X} cy={svgHeight - PAD_Y} r={DOT_R} fill="black" />}
        </svg>
      );
    });
  };
  ...
  const renderRightATTLine = () => {
    const circleSize = 10;
    const totalSpacing = circleSize + stationGap;
    const midMarginLeft = 70;

    const DOT_R = 5;
    const LINE_X = DOT_R;
    const SVG_W  = LINE_X + DOT_R;
    const PAD_Y  = DOT_R;
    ...
    return stationsRight.slice(0, -1).map((station, idx) => {
      ...
      const svgTop    = startY - PAD_Y;
      const svgHeight = segH + PAD_Y * 2;
      ...
      return (
        <svg
          key={`att-right-${station.code}-${next.code}`}
          style={{
            position: 'absolute',
            marginLeft: midMarginLeft,  // keep using marginLeft for alignment
            top: svgTop,
            pointerEvents: 'none',
            overflow: 'visible',
            zIndex: 2,
        ...
        >
          ...
          />
          {showStartDot && <circle cx={LINE_X} cy={PAD_Y + 1} r={DOT_R} fill="black" />}
          {showEndDot   && <circle cx={LINE_X} cy={svgHeight - PAD_Y + 1} r={DOT_R} fill="black" />}
        </svg>
      );
    });
  };
  ...
  const renderLeftInnerBusLine = () => {
    ...
    return stationsLeft.slice(0, -1).map((station, idx) => {
      ...
      const right = busRanges.length > 0 && attRanges.length > 0? 49: 61;
      const startY = idx * totalSpacing + circleSize / 2;
      const endY = (idx + 1) * totalSpacing + circleSize / 2;
  ...
  const renderRightInnerBusLine = () => {
    ...
    return stationsRight.slice(0, -1).map((station, idx) => {
      ...
      const lineColor = "#dd3063";
      if (!(busSet.has(station.code) && busSet.has(next.code))) return null;
      const left = busRanges.length > 0 && attRanges.length > 0? 61: 71;
      const startY = idx * totalSpacing + circleSize / 2;
      const endY = (idx + 1) * totalSpacing + circleSize / 2;
      const height = endY - startY;
  ...
  const renderUShapeInnerBusLine = () => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const path = bezierPathString([q0, q1, q2, q3]);
      segments.push(
        <path key={`inner-bus-${i}`} d={path} fill="transparent" stroke="#dd3063" strokeWidth="2" />
      );
    }
  ...
  const renderUShapeLines = () => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const lineColor = '#005ec4';
      const lineType = '🔵 Normal';
  ...
  const renderUShapeStations = () => {
    const stationNodes = uStations.map((station, idx) => {
      const uSegmentCount = uStations.length + 1;
      const tValue = Array.from({ length: uSegmentCount + 1 }, (_, i) => i / uSegmentCount);
      const t = tValue[idx + 1];
      ...
      return (
        <g key={station.code}>
          {/* station marker */}
          (
            <circle cx={x} cy={y} r={5} fill="#005ec4" />
          )
          ...
          {lineIcon && (
            <>
              <circle cx={liX + 5} cy={liY + 5} r={10} fill="#fff" />
              <image
                x={liX}
                y={liY}
                width={12}
              ...
              />
              <circle cx={liX + 5} cy={liY + 5} r={10} fill="none" stroke="#fff" strokeWidth="0.5" />
            </>
          )}
        </g>
      );
  ...
  return (
    <div style={{ paddingTop: 0, width: 420, minHeight: 870, position: 'relative' }}>
      <Header type="Delay" from={formatStationLabel(attRanges[0]?.from ?? '')} to={formatStationLabel(attRanges.at(-1)?.to ?? '')} />
